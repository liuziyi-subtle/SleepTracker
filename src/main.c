...
int main(int argc, char **argv) {
    ...
    if (d) {
        ...
        while ((dir = readdir(d)) != NULL) {
            ...
            // Input data buffer.
            struct LSSleepData *data = (struct LSSleepData*) data_malloc(sizeof(struct LSSleepData) * 60);
            uint16_t dataCounter;
            // 取出睡眠深度结果
            struct LSSleepResult *result = (struct LSSleepResult*) data_malloc(sizeof(struct LSSleepResult));
...
//                        for (uint16_t j = 0; j < result->numSleepSegments; ++j) {
//                            for (uint32_t k = 0; k < result->sleepSegments[j].duration; ++k) {
//                                PLOT.depthBuf[PLOT.depthBufLen++] = result->sleepSegments[j].depth;
//                            }
//                        }
                    }
                    dataCounter = 0;
...
//            sprintf(depthPath, "%s/%s_depth.csv", argv[2], fileNamePrefix);
//            WriteInt(depthPath, PLOT.depthBuf, PLOT.depthBufLen);
//            printf("PLOT.depthBufLen: %d\n", PLOT.depthBufLen);
//            PLOT.depthBufLen = 0;
//            data_free(depthPath);
