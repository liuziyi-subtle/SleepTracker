...
//static PLOT_STRUC PLOT;
#else  // 手表运行
//#include "FreeRTOS.h"
//#include "ls_log.h"
//#include "rtos.h"
#define data_malloc malloc//pvPortMalloc
#define data_free free//vPortFree
#endif  // DEBUG_LOCAL
...
typedef enum
{
    ...
    ORI_SLEEP_DURATION = 60,
    ALLDAY_SLEEP_DURATION = 30,
    NAP_SLEEP_DURATION = 17,
}alg_sleep_duration_t;
alg_SleepInfo_t m_sleep_setting;
static uint32_t m_utc_sleep_end;
...
//    if((cur_hour < tmp.lsAwake_start_hr) || (cur_hour > tmp.lsAwake_end_hr))
//    {
//        prob_line = 0.5
//    }
//    else
...
//    }
  ...
  //20210421 ByJames
  alg_IsAwake_t* tmp =  Get_IsAwakeInfo_ByRef();
  uint8_t cur_hour = tmp->cur_hour;
  tmp->prob = prob;
  if((m_sleep_setting.lsAwake_start_hr <= cur_hour) && (cur_hour <= m_sleep_setting.lsAwake_end_hr))
  {
      pred = (tmp->IsAwake_log == IS_AWAKE_NO) ? 1:0;
      debug_p(" predByJames: %d ", pred);
  }
...
//  }
  ...
  if((m_sleep_setting.lsAwake_start_hr <= cur_hour) && (cur_hour <= m_sleep_setting.lsAwake_end_hr))
  {
      switch(m_sleep_setting.cur_mode)
      {
          case NAP_MODE:
              m_sleep_setting.min_sleep_duration = NAP_SLEEP_DURATION;
              break;
          case ALLDAY_MODE:
              m_sleep_setting.min_sleep_duration = ALLDAY_SLEEP_DURATION;
              break;
          default:
              m_sleep_setting.min_sleep_duration = ALLDAY_SLEEP_DURATION;
              break;
      ...
      sleepStatus = WindowAverage_j(averageWindow, aPtr, SLEEP_STATUS_AVG_WINDOW_LEN, info->utcTime, &m_utc_sleep_end);
  }
  else
  {
      switch(m_sleep_setting.cur_mode)
      {
          case ORI_MODE:
              m_sleep_setting.min_sleep_duration = ORI_SLEEP_DURATION;
              break;
          case ALLDAY_MODE:
              m_sleep_setting.min_sleep_duration = ALLDAY_SLEEP_DURATION;
              break;
      ...
      sleepStatus = WindowAverage(averageWindow, SLEEP_STATUS_AVG_WINDOW_LEN);
  }
...
//  PLOT.wearIndBuf[PLOT.wearIndBufLen++] = wearIndicatorBuf[wPtr];
  ...
  if ((sleepStatus && gSleepMarkerBufLen % 2 == 0) ||
    ...
    //Odd: Sleep segment, Even: Wake segment.

  }


  debug_p(" sleepStatus:%d, pred:%d %f, gSleepCycleDetector:%d", sleepStatus, pred, prob, gSleepCycleDetector);
...
void LSSleepGetResult(struct LSSleepResult *result) {
  ...
  debug_p(" totalSleepDuration: %d", totalSleepDuration);


  // TODO(Ziyi Liu): MIN_SLEEP_DURATION在心率灯关闭时需要延长至2h以便适应2h内
  //  gMeanDiffSum的判定。
  // TODO(Ziyi Liu): T_COMP和T_TEMP的操作需要位于整个result的计算之后。
  if (totalSleepDuration < m_sleep_setting.min_sleep_duration ) {
  ...
  result->numSleepSegments = numSleepSegments;
    ...
    if((m_sleep_setting.lsAwake_start_hr <= cur_hour) && (cur_hour <= m_sleep_setting.lsAwake_end_hr))
    {
        result->getupTimeUtc = m_utc_sleep_end;
    }
...
//20210421 ByJames
alg_SleepInfo_t* alg_SetSleepMode(alg_SleepMode sleep_mode)
{
    m_sleep_setting.cur_mode = sleep_mode;
    switch(sleep_mode)
    {
        case UNKNOWN_MODE:
        ...
        case ORI_MODE:
            m_sleep_setting.min_sleep_duration = ORI_SLEEP_DURATION;
            m_sleep_setting.lsAwake_start_hr = 100;
            m_sleep_setting.lsAwake_end_hr = 0;
            break;
        case NAP_MODE:
            m_sleep_setting.min_sleep_duration = NAP_SLEEP_DURATION;
            m_sleep_setting.lsAwake_start_hr = 12;
            m_sleep_setting.lsAwake_end_hr = 15;
            break;
        case ALLDAY_MODE:
            m_sleep_setting.min_sleep_duration = ALLDAY_SLEEP_DURATION;
            m_sleep_setting.lsAwake_start_hr = 0;
            m_sleep_setting.lsAwake_end_hr = 100;
            break;
        default:
    ...
    if(m_sleep_setting.min_sleep_duration > MAX_SLEEP_DURATION)
    {
        m_sleep_setting.min_sleep_duration = MAX_SLEEP_DURATION;
    }
    else if(m_sleep_setting.min_sleep_duration < NAP_SLEEP_DURATION)
    {
        m_sleep_setting.min_sleep_duration = NAP_SLEEP_DURATION;
    }
    return &m_sleep_setting;
}