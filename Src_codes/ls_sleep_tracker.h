...
// 最短睡眠时长
#define MIN_SLEEP_DURATION (20)  //60
// 睡眠深度 - 清醒
#define D_AWAKE (1)
// 睡眠深度 - 眼动
#define D_REM (4)
...
typedef enum
{
    UNKNOWN_MODE,          /**< 空集合，不做任何修改 */
    ORI_MODE,              /**< 回到以前的睡眠算法 */
    NAP_MODE,              /**< 午睡时，可自动侦测午睡，入睡条件为30分钟，其他时段为60分钟 */
    ALLDAY_MODE,           /**< 全天自动侦测30分钟睡眠 */
}alg_SleepMode;

/** @struct alg_SleepInfo_t
*   @brief 睡眠参数设置结构体，重要参数，修改前请先找维护人
*/
typedef struct {
    uint16_t min_sleep_duration;
    uint8_t lsAwake_start_hr;
    uint8_t lsAwake_end_hr;
    alg_SleepMode cur_mode;
} alg_SleepInfo_t;
